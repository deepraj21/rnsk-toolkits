import { tool } from 'ai';
import { z } from 'zod';
import { DescribeConformancePackStatusCommand } from '@aws-sdk/client-config-service';
...
export const awsDescribeConformancePackStatus = tool({
  ...
  inputSchema: z.object({
    ...
    conformancePackNames: z.array(z.string()).optional().describe('List of conformance pack names'),
    limit: z.number().optional().describe('Maximum number of results to return'),
    nextToken: z.string().optional().describe('Token for pagination'),
  }),
  execute: async ({ awsCredentials, region, conformancePackNames, limit, nextToken }) => {
    if (!awsCredentials) {
