import { tool } from 'ai';
import { z } from 'zod';
import { GetRegistryScanningConfigurationCommand } from '@aws-sdk/client-ecr';
import { createEcrClient } from '../client.js';

export const awsGetRegistryScanningConfiguration = tool({
  ...
  execute: async ({ awsCredentials, region }) => {
    ...
    try {
      ...
      return { error: 'Failed to get the registry scanning configuration', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
