...
export const awsGetLifecyclePolicy = tool({
  ...
  execute: async ({ awsCredentials, region, repositoryName, registryId }) => {
    ...
    try {
      ...
      return { error: 'Failed to get the lifecycle policy for an ECR repository', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
