...
export const awsGetReplicationConfiguration = tool({
  ...
  execute: async ({ awsCredentials, region }) => {
    ...
    try {
      ...
      return { error: 'Failed to get the replication configuration for the registry', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
