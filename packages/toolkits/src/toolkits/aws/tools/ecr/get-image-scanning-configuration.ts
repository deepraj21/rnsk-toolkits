...
export const awsGetImageScanningConfiguration = tool({
  ...
  execute: async ({ awsCredentials, region, repositoryName, registryId }) => {
    ...
    try {
      ...
      return { error: 'Failed to get the image scanning configuration for an ECR repository', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
