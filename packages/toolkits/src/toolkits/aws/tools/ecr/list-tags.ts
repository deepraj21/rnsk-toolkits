import { tool } from 'ai';
import { z } from 'zod';
import { ListTagsForResourceCommand } from '@aws-sdk/client-ecr';
import { createEcrClient } from '../client.js';

export const awsListEcrTags = tool({
  ...
  execute: async ({ awsCredentials, region, resourceArn }) => {
    ...
    try {
      ...
      return { error: 'Failed to list tags for an ECR resource', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
