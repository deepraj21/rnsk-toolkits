import { tool } from 'ai';
import { z } from 'zod';
import { ListTrainingJobsCommand } from '@aws-sdk/client-sagemaker';
import { createSageMakerClient } from '../client.js';

export const awsListSagemakerTrainingJobs = tool({
  ...
  execute: async ({ awsCredentials, region, nextToken, maxResults, creationTimeAfter, creationTimeBefore, lastModifiedTimeAfter, lastModifiedTimeBefore, nameContains, statusEquals, sortBy, sortOrder }) => {
    ...
    try {
      ...
      const command = new ListTrainingJobsCommand({
          NextToken: nextToken,
      ...
      return {
                  trainingJobSummaries: response.TrainingJobSummaries || [],
                  nextToken: response.NextToken,
              };
    } catch (err) {
      return { error: 'Failed to list SageMaker training jobs', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
