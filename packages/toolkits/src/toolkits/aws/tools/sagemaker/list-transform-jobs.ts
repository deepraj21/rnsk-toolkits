import { tool } from 'ai';
import { z } from 'zod';
import { ListTransformJobsCommand } from '@aws-sdk/client-sagemaker';
import { createSageMakerClient } from '../client.js';

export const awsListSagemakerTransformJobs = tool({
  ...
  execute: async ({ awsCredentials, region, nextToken, maxResults, creationTimeAfter, creationTimeBefore, lastModifiedTimeAfter, lastModifiedTimeBefore, nameContains, statusEquals, sortBy, sortOrder }) => {
    ...
    try {
      ...
      const command = new ListTransformJobsCommand({
          NextToken: nextToken,
      ...
      return {
                  transformJobSummaries: response.TransformJobSummaries || [],
                  nextToken: response.NextToken,
              };
    } catch (err) {
      return { error: 'Failed to list SageMaker transform jobs', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
