import { tool } from 'ai';
import { z } from 'zod';
import { ListHyperParameterTuningJobsCommand } from '@aws-sdk/client-sagemaker';
import { createSageMakerClient } from '../client.js';

export const awsListSagemakerHyperparameterTuningJobs = tool({
  ...
  execute: async ({ awsCredentials, region, nextToken, maxResults, creationTimeAfter, creationTimeBefore, lastModifiedTimeAfter, lastModifiedTimeBefore, nameContains, statusEquals, sortBy, sortOrder }) => {
    ...
    try {
      ...
      const command = new ListHyperParameterTuningJobsCommand({
          NextToken: nextToken,
      ...
      return {
                  hyperParameterTuningJobSummaries: response.HyperParameterTuningJobSummaries || [],
                  nextToken: response.NextToken,
              };
    } catch (err) {
      return { error: 'Failed to list SageMaker hyperparameter tuning jobs', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
