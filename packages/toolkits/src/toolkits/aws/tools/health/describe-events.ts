...
export const awsDescribeHealthEvents = tool({
  ...
  execute: async ({ awsCredentials, region, filter, nextToken, maxResults, locale }) => {
    ...
    try {
      ...
      return { error: 'Failed to get information about events that affect your AWS resources', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
