import { tool } from 'ai';
import { z } from 'zod';
import { UpdateEndpointCommand } from '@aws-sdk/client-eventbridge';
import { createEventBridgeClient } from '../client.js';

export const awsUpdateEventbridgeEndpoint = tool({
  ...
  inputSchema: z.object({
    ...
    routingConfig: z.record(z.any()).optional().describe('Routing configuration'),
    replicationConfig: z.record(z.any()).optional().describe('Replication configuration'),
    eventBuses: z.array(z.record(z.any())).optional().describe('Event buses'),
  ...
  execute: async ({ awsCredentials, region, name, description, routingConfig, replicationConfig, eventBuses, roleArn }) => {
    ...
    try {
      ...
      return { error: 'Failed to update an EventBridge endpoint', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
