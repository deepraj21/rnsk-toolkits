...
export const awsGetCostAndUsageWithResources = tool({
  ...
  execute: async ({ awsCredentials, region, timePeriod, granularity, metrics, filter, groupBy, nextPageToken }) => {
    ...
    try {
      ...
      return { error: 'Failed to retrieves cost and usage metrics with resources', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
