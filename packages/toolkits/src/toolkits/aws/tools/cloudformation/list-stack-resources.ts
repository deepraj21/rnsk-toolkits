...
export const awsListCloudformationStackResources = tool({
  ...
  execute: async ({ awsCredentials, region, stackName, nextToken }) => {
    ...
    try {
      ...
      return { stackResourceSummaries: response.StackResourceSummaries, nextToken: response.NextToken };
    } catch (err) {
      return { error: 'Failed to list all resources in a CloudFormation stack', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
