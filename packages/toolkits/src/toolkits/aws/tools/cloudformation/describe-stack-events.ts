...
export const awsDescribeCloudformationStackEvents = tool({
  ...
  execute: async ({ awsCredentials, region, stackName, nextToken }) => {
    ...
    try {
      ...
      return { stackEvents: response.StackEvents, nextToken: response.NextToken };
    } catch (err) {
      return { error: 'Failed to describe events for a CloudFormation stack', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
