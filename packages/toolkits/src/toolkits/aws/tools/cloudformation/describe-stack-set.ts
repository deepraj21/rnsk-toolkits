import { tool } from 'ai';
import { z } from 'zod';
import { DescribeStackSetCommand } from '@aws-sdk/client-cloudformation';
import { createCloudFormationClient } from '../client.js';

export const awsDescribeCloudformationStackSet = tool({
  description: 'Describe a CloudFormation stack set. Use it to inspect current state before making changes.',
  inputSchema: z.object({
    awsCredentials: z.string().optional().describe('Injected by system; do not provide'),
    region: z.string().optional().describe('AWS region to query (default: us-east-1)'),
    stackSetName: z.string().describe('Name of the stack set'),
  }),
  execute: async ({ awsCredentials, region, stackSetName }) => {
    if (!awsCredentials) {
      return { error: 'AWS credentials are required. Connect AWS first.' };
    }
    try {
      const client = createCloudFormationClient(awsCredentials, region);

      const command = new DescribeStackSetCommand({
          StackSetName: stackSetName,
      });
      const response = await client.send(command);
      return { stackSet: response.StackSet };
    } catch (err) {
      return { error: 'Failed to describe a CloudFormation stack set', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
