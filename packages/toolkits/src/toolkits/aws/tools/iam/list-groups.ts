...
export const awsListIamGroups = tool({
  ...
  inputSchema: z.object({
    ...
    maxItems: z.number().optional().describe('Maximum number of groups to return'),
    marker: z.string().optional().describe('Pagination marker'),
    pathPrefix: z.string().optional().describe('Path prefix to filter groups'),
  }),
  execute: async ({ awsCredentials, region, maxItems, marker, pathPrefix }) => {
    ...
    try {
      ...
      return { error: 'Failed to list all IAM groups in the AWS account', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
