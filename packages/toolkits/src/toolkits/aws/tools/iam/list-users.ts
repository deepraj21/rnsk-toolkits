...
export const awsListIamUsers = tool({
  ...
  inputSchema: z.object({
    ...
    maxItems: z.number().optional().describe('Maximum number of users to return (1-1000)'),
    marker: z.string().optional().describe('Pagination marker from previous response'),
    pathPrefix: z.string().optional().describe('Path prefix to filter users (e.g., /division_abc/)'),
  }),
  execute: async ({ awsCredentials, region, maxItems, marker, pathPrefix }) => {
    ...
    try {
      ...
      return { error: 'Failed to list all IAM users in the AWS account', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
