...
export const awsDeleteIamPolicy = tool({
  ...
  execute: async ({ awsCredentials, region, policyArn }) => {
    ...
    try {
      ...
      return { error: 'Failed to delete a customer managed policy', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
