import { tool } from 'ai';
import { z } from 'zod';
import { CreateRealtimeLogConfigCommand } from '@aws-sdk/client-cloudfront';
import { createCloudFrontClient } from '../client.js';

export const awsCreateCloudfrontRealtimeLogConfig = tool({
  ...
  execute: async ({ awsCredentials, region, endPoints, fields, name, samplingRate }) => {
    ...
    try {
      ...
      return { error: 'Failed to create a CloudFront real-time log config', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
