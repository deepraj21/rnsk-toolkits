import { tool } from 'ai';
import { z } from 'zod';
import { ListContinuousDeploymentPoliciesCommand } from '@aws-sdk/client-cloudfront';
import { createCloudFrontClient } from '../client.js';

export const awsListCloudfrontContinuousDeploymentPolicies = tool({
  ...
  execute: async ({ awsCredentials, region, marker, maxItems }) => {
    ...
    try {
      ...
      return {
                  continuousDeploymentPolicyList: response.ContinuousDeploymentPolicyList,
              };
    } catch (err) {
      return { error: 'Failed to list all CloudFront continuous deployment policies', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
