import { tool } from 'ai';
import { z } from 'zod';
import { DeleteCachePolicyCommand } from '@aws-sdk/client-cloudfront';
import { createCloudFrontClient } from '../client.js';

export const awsDeleteCloudfrontCachePolicy = tool({
  ...
  execute: async ({ awsCredentials, region, id, ifMatch }) => {
    ...
    try {
      ...
      return {
                  success: true,
                  message: `Cache policy ${id} deleted successfully`,
              };
    } catch (err) {
      return { error: 'Failed to delete a CloudFront cache policy', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
