...
export const awsGetS3BucketPolicyStatus = tool({
  ...
  execute: async ({ awsCredentials, region, bucket }) => {
    ...
    try {
      ...
      return { policyStatus: response.PolicyStatus };
    } catch (err) {
      return { error: 'Failed to get the policy status for an S3 bucket', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
