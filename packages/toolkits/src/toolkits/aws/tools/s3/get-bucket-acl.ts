...
export const awsGetS3BucketAcl = tool({
  ...
  execute: async ({ awsCredentials, region, bucket }) => {
    ...
    try {
      ...
      return { error: 'Failed to get the ACL (Access Control List) for an S3 bucket', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
