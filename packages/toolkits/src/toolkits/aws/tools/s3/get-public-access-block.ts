...
export const awsGetS3PublicAccessBlock = tool({
  ...
  execute: async ({ awsCredentials, region, bucket }) => {
    ...
    try {
      ...
      return { publicAccessBlockConfiguration: response.PublicAccessBlockConfiguration };
    } catch (err) {
      return { error: 'Failed to get public access block configuration for an S3 bucket', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
