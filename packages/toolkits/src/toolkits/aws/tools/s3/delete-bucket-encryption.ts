...
export const awsDeleteS3BucketEncryption = tool({
  ...
  execute: async ({ awsCredentials, region, bucket }) => {
    ...
    try {
      ...
      const command = new DeleteBucketEncryptionCommand({ Bucket: bucket });
      await client.send(command);
      return { success: true };
    } catch (err) {
      return { error: 'Failed to delete the encryption configuration for an S3 bucket', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
