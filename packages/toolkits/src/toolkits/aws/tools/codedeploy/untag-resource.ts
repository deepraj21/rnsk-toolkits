import { tool } from 'ai';
import { z } from 'zod';
import { UntagResourceCommand } from '@aws-sdk/client-codedeploy';
import { createCodeDeployClient } from '../client.js';

export const awsUntagCodedeployResource = tool({
  ...
  execute: async ({ awsCredentials, region, resourceArn, tagKeys }) => {
    ...
    try {
      ...
      return { error: 'Failed to remove tags from a CodeDeploy resource', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
