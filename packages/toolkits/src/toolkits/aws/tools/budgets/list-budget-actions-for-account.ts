import { tool } from 'ai';
import { z } from 'zod';
import { DescribeBudgetActionsForAccountCommand } from '@aws-sdk/client-budgets';
import { createBudgetsClient } from '../client.js';

export const awsListBudgetActionsForAccount = tool({
  ...
  execute: async ({ awsCredentials, region, accountId, maxResults, nextToken }) => {
    ...
    try {
      ...
      const command = new DescribeBudgetActionsForAccountCommand({
          AccountId: accountId,
      ...
      return { error: 'Failed to list all budget actions for an account', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
