import { tool } from 'ai';
import { z } from 'zod';
import { ListBuildsForProjectCommand } from '@aws-sdk/client-codebuild';
import { createCodeBuildClient } from '../client.js';

export const awsListCodebuildBuildsForProject = tool({
  ...
  execute: async ({ awsCredentials, region, projectName, sortOrder, nextToken }) => {
    if (!awsCredentials) {
    ...
    try {
      ...
      return { error: 'Failed to list build IDs for a project', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
