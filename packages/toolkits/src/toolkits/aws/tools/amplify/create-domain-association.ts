import { tool } from 'ai';
import { z } from 'zod';
import { CreateDomainAssociationCommand } from '@aws-sdk/client-amplify';
import { createAmplifyClient } from '../client.js';

export const awsCreateAmplifyDomainAssociation = tool({
  ...
  execute: async ({ awsCredentials, region, appId, domainName, enableAutoSubDomain, subDomainSettings, autoSubDomainCreationPatterns, autoSubDomainIAMRole }) => {
    ...
    try {
      ...
      return { error: 'Failed to creates a new domain association for an Amplify app', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
