import { tool } from 'ai';
import { z } from 'zod';
import { ListTagsForResourceCommand } from '@aws-sdk/client-eks';
import { createEksClient } from '../client.js';

export const awsListEksTags = tool({
  ...
  execute: async ({ awsCredentials, region, resourceArn }) => {
    ...
    try {
      ...
      return { error: 'Failed to list tags for an EKS resource', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
