import { tool } from 'ai';
import { z } from 'zod';
import { UntagResourceCommand } from '@aws-sdk/client-codepipeline';
import { createCodePipelineClient } from '../client.js';

export const awsUntagCodepipelineResource = tool({
  ...
  execute: async ({ awsCredentials, region, resourceArn, tagKeys }) => {
    ...
    try {
      ...
      return { error: 'Failed to remove tags from a CodePipeline resource', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
