import { tool } from 'ai';
import { z } from 'zod';
import { TagResourceCommand } from '@aws-sdk/client-backup';
import { createBackupClient } from '../client.js';

export const awsTagBackupResource = tool({
  ...
  inputSchema: z.object({
    ...
    tags: z.record(z.any()).describe('Tags to apply'),
  }),
  execute: async ({ awsCredentials, region, resourceArn, tags }) => {
    ...
    try {
      ...
      return { error: 'Failed to add tags to a backup resource', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
