import { tool } from 'ai';
import { z } from 'zod';
import { DescribeRepositoryCommand } from '@aws-sdk/client-codeartifact';
import { createCodeArtifactClient } from '../client.js';

export const awsDescribeCodeartifactRepository = tool({
  ...
  execute: async ({ awsCredentials, region, domain, domainOwner, repository }) => {
    ...
    try {
      ...
      return { error: 'Failed to get details about a CodeArtifact repository', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
