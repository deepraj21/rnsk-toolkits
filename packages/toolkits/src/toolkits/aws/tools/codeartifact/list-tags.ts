import { tool } from 'ai';
import { z } from 'zod';
import { ListTagsForResourceCommand } from '@aws-sdk/client-codeartifact';
import { createCodeArtifactClient } from '../client.js';

export const awsListCodeartifactTags = tool({
  ...
  execute: async ({ awsCredentials, region, resourceArn }) => {
    ...
    try {
      ...
      return { error: 'Failed to list tags for a CodeArtifact resource', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
