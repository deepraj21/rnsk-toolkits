import { tool } from 'ai';
import { z } from 'zod';
import { DeletePackageGroupCommand } from '@aws-sdk/client-codeartifact';
import { createCodeArtifactClient } from '../client.js';

export const awsDeleteCodeartifactPackageGroup = tool({
  ...
  inputSchema: z.object({
    ...
    packageGroup: z.string().describe('The name of the package group to delete'),
  }),
  execute: async ({ awsCredentials, region, domain, domainOwner, packageGroup }) => {
    ...
    try {
      ...
      const command = new DeletePackageGroupCommand({
          domain: domain,
      ...
      return { error: 'Failed to delete a CodeArtifact package group', message: err instanceof Error ? err.message : 'Unknown error' };
    }
  },
});
