...
export const getTeamStyles = tool({
    description:
        'Lists published styles (fills, text, effects, grids) in a team library with pagination. Empty list means nothing published.',
    inputSchema: z.object({
        figmaToken: figmaTokenField,
        teamId: z.string().describe('Team ID from the team page URL'),
        pageSize: z.number().optional().describe('Styles per page (max 100, default 30)'),
    ...
    execute: async ({ figmaToken, teamId, pageSize, after, before }) => {
        try {
            ...
            const result = await figmaRequest(figmaToken, `/v1/teams/${teamId}/styles`, {
                query: { page_size: pageSize, after, before },
            });
            if (!result.ok) return { error: 'Failed to get team styles', details: result.error };
            return result.data;
        } catch (error) {
            return {
                error: 'Error getting team styles',
