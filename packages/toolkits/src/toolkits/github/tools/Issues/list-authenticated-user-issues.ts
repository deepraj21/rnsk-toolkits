...
export const listAuthenticatedUserIssues = tool({
    description:
        'List issues assigned to the authenticated user across all visible repositories. Use when the user wants to see their assigned issues.',
    ...
    execute: async ({ githubToken, ...params }) => {
        ...
        const { data } = await octokit.rest.issues.listForAuthenticatedUser(params);
