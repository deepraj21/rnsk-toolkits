...
export const listOrgIssues = tool({
    description:
        'List issues assigned to the authenticated user for a specific organization across all repositories in that organization.',
    ...
    execute: async ({ githubToken, org, ...params }) => {
        if (!githubToken) {
