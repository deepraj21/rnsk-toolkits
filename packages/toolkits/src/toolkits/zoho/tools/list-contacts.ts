...
export const ZohoListContacts = tool({
    ...
    inputSchema: z.object({
        ...
        ids: z.string().optional().describe('Comma-separated contact record IDs to fetch specific contacts. Note: This parameter is ignored when page_token is provided.'),
        cvid: z.number().int().optional().describe('Custom view ID to filter the contact records. Cannot be used together with sort_by. Note: This parameter is ignored when page_token is provided.'),
        ...
        fields: z.string().min(1).optional().describe('Comma-separated API names of the contact fields to retrieve (max 50). At least one field must be specified (empty string not allowed). Common contact fields: First_Name, Last_Name, Email, Phone, Mobil'),
        sort_by: z.string().optional().describe('Valid fields for sorting Zoho CRM contact records.'),
        per_page: z.number().int().min(1).max(200).optional().describe('Number of contact records per page. Maximum is 200. Applies to both page and page_token pagination.'),
        page_token: z.string().optional().describe('Token-based pagination parameter to fetch contacts beyond the 2,000-record discrete limit. Use the info.next_page_token from the previous response. Cannot be used together with page.'),
        sort_order: z.string().optional().describe('Sort order for contact records.'),
    }),
    execute: async (params) => {
        ...
        try {
            ...
            return { error: 'Error in ZohoListContacts', message: e instanceof Error ? e.message : 'Unknown error' };
        }
    },
});
